...
const allMessages = StableBTreeMap<string, Message[]>(1);
...
export default Server(() => {
  ...
  app.post("/add/conversation", (req: Request, res: Response) => {
    ...
    // Add the new message to allMessages
    const existingMessages = allMessages.get(payload.userIdentity);
    const updatedAllMessages = existingMessages.Some 
      ? [...existingMessages.Some, newMessage] 
      : [newMessage];
    allMessages.insert(payload.userIdentity, updatedAllMessages);
  ...
  app.get("/all-messages/:userIdentity", (req: Request, res: Response) => {
    const userIdentity = req.params.userIdentity;
    const messages = allMessages.get(userIdentity);
    if ("None" in messages) {
      return res.status(404).json({ message: `No messages found for ${userIdentity}` });
    }
    return res.status(200).json(messages.Some);
  });
